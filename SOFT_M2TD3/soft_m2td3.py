...
class SoftM2TD3:
    ...
    def __init__(
        ...
        ) / (self.config["algorithm"]["max_steps"] / 2)
        ...
        self.update_w = [0 for _ in range(len(self.max_omega))]
    ...
    def _init_network(self, state_dim, action_dim, omega_dim, max_action, config):
        ...
        self.critic_target_p = copy.deepcopy(self.critic_network)
    ...
    def _calc_sum_policy_loss(self, policy_loss_list):
        '''Compute sum of policy losses

        Parameters
        ----------
        policy_loss_list : List[torch.Tensor]
            policy loss list

        '''

        sum_policy_loss = None
        pks = self.hatomega_prob
        for pk, policy_loss in zip(pks, policy_loss_list):
            if sum_policy_loss is None:
                sum_policy_loss = pk * policy_loss
            else:
                sum_policy_loss += pk * policy_loss
        self.update_w = pks
    ...
    def _update_actor(self, state_batch):
        ...
        sum_policy_loss = self._calc_sum_policy_loss(policy_loss_list)
    ...
    def _update_target(self):
        ...
        for target_param, param in zip(
            self.critic_target_p.parameters(), self.critic_network.parameters()
        ):
    ...
    def _calc_diff(self):
        ...
        for hatomega_pair in itertools.combinations(hatomega_list, 2):
            distance = np.linalg.norm(
                np.array(hatomega_pair[0]) - np.array(hatomega_pair[1]), ord=1
            )
            if distance <= self.config["algorithm"]["hatomega_distance"]:
                change_hatomega_index_list.append(
                    hatomega_list.index(hatomega_pair[0])
                )
    ...
    def _select_hatomega(self):
        ...
        select_index = np.random.choice(a=self.element_list, size=1, p=self.hatomega_prob)
