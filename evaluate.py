...
def evaluate(
    ...
    state_dim : int
        dimension of state
    iteration : int
        Iteration number of the policy
    f : file
        File object to write the result to
    ...
    for evaluate_point in evaluate_points:
        ...
        f.write(f"{iteration},{evaluate_point},{reward_str}\n")
...
def main(
    ...
    interval : int
        Frequency of evaluating policy
    max_iteration : int
        Max iteration of evaluating policy
    num_evaluate_point : int
        Number to partition the uncertainty parameter set
    dim_evaluate_point : int
        Dimension of uncertainty parameter
    ...
    for iteration in range(max_iteration + 1):
        if iteration % interval == 0:
            evaluate_iterations.append(iteration)
